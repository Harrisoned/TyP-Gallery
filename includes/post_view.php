<?php
	...
	else
	{
		...
		echo '<div class="sidebar">
            ...
            <center>
                <div class="sidebar3">
                    <center class="search_list"><b>Search for images</b> <img src="layout/icons/question-small-white.png" alt="Supports wildcards *" title="Supports wildcards (*) , negating tags (-) and fuzzy search (~)"/></center>
		...
		echo '</ul><li style="padding-top: 10px;"></li>
                <div>
                    ...
                    <ul>
                        ...
                        <li><a href="#" onClick="presizem()">Resize image</a></li>
                        <li><a href="#" onclick="changeAvatar(\''.$id.'\'); return false;">Set as avatar</a></li>
                        <li><a href="images/'.$post_data['directory'].'/'.$post_data['image'].'">Original image</a></li>
                        <li><a href="#" onclick="Note.create('.$id.'); return false;">Add note</a></li>
                        <li><a href="#" onclick="if(confirm(\'Are you sure you want to delete this post?\')){var f = document.createElement(\'form\'); f.style.display = \'none\'; this.parentNode.appendChild(f); f.method = \'POST\'; f.action = \'./public/remove.php?id='.$id.'&amp;removepost=1\'; f.submit();}; return false;">Delete</a></li>';
        if($post_data['spam'] == false){
            echo '<li><div id="pfd"><a href="#" onClick="pflag(\''.$id.'\'); return false;">Flag for deletion</a></div></li>';
        } else {
			print '<li><b>Post reported</b></li>';
        }

        echo '<li><a href="#" onclick="addFav(\''.$id.'\'); return false;">Add to favorites</a></li>
                </ul>
                </div>
                <div>
                    <h5>History</h5><br />
                    <ul>
				        ...
				        <li><a href="#" onclick="document.location=\'index.php?page=history&amp;type=page_notes&amp;id='.$id.'\'; return false;">Notes</a></li>
                    </ul>
                </div>
                <div>
                    <h5>Related Posts</h5><br />
                    <ul>'; ?>
                    <?php $prev_next['0'] != "" ? print '<li><a href="index.php?page=post&amp;s=view&amp;id='.$prev_next['0'].'">Previous</a></li>' : print ""; $prev_next['1'] != "" ? print '<li><a href="index.php?page=post&amp;s=view&amp;id='.$prev_next['1'].'">Next</a></li>' : print ""; $row['parent'] == 0 ? print "<br />" : print "<br /><li><a href=\"index.php?page=post&s=view&id=".$row['parent']."\">Parent</a></li>";
                    ?>
        <?php
              echo '</ul>
                </div>
                </div></div></div></div></center>
                <div class="content" id="right-col"  style="padding-left: 10px;"><div><div id="note-container">
                <div id="post-view">';
		...
		while($retme = $note_data->fetch_assoc())
		{
			...
			<div class="note-corner" id="note-corner-'.$retme['id'].'"></div></div>
        ...
        if(intval($post_data['width']) > 1000){
            echo '<img alt="img" src="images/'.$post_data['directory'].'/'.$post_data['image'].'" id="image" class="resize" onclick="Note.toggle();" />';
        } else {
            echo '<img alt="img" src="images/'.$post_data['directory'].'/'.$post_data['image'].'" id="image" onclick="Note.toggle();" />';
        }
        
        echo '<p id="note-count"></p>
