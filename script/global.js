...
function changeAvatar(id)
{
	...
	HttpRequest.onreadystatechange=function()
	{
		if(HttpRequest.readyState==4)
		{
			...
			{
				notice("Avatar changed");
			}
		}
	}
	HttpRequest.open("GET","public/addavatar.php?id=" + id,true);
	HttpRequest.send(null);
}
...
function filterPosts(posts) {
	...
	if (ignored.length > 0) {
        ...
        document.getElementById("blacklisted-sidebar").style.display = "";
	}
}
...
function presizem() {
    document.getElementById("image").className = "resize";
    document.getElementById("resized_notice").style.display = "";
}